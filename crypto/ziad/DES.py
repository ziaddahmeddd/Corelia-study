...
def sbox_substitution(xor_result):
    sbox_str = ""
    for i in range(8):
        row = bin2dec(xor_result[i * 6] + xor_result[i * 6 + 5])
        col = bin2dec(xor_result[i * 6 + 1:i * 6 + 5])
...
def des_round(left, right, round_key):
    ...
    sbox_permuted = permute(sbox_str, P, 32)
...
def des_encrypt(plain_text, round_keys):
    ...
    right = plain_text[32:]
    for i in range(16):
        new_right = des_round(left, right, round_keys[i])
        left = right
        right = new_right
    combined_text = right + left
    cipher_text = permute(combined_text, IP_INV, 64)
    return cipher_text
...
def des_decrypt(cipher_text, round_keys):
    ...
    for i in range(15, -1, -1):
        ...
        right = new_right
    combined_text = right + left
    plain_text = permute(combined_text, IP_INV, 64)
    return plain_text

def main():
    choice = input("Choose (E)ncryption or (D)ecryption: ").strip().upper()
    key = input("Enter the 16-character hexadecimal key: ").strip()
    if choice == 'E':
        plain_text = input("Enter the 16-character hexadecimal plaintext: ").strip()
        ...
        cipher_text = input("Enter the 16-character hexadecimal ciphertext: ").strip()
        ...
        plain_text = des_decrypt(cipher_text, round_keys)
        print("Decrypted Text:", bin2hex(plain_text))
    else:
